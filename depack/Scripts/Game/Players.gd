...
func get_closest_enemy_player(my_pos:Vector3, dont_target_player_w_id:int, my_team_name:String, is_bot:bool = false, search_dist:float = 9999)->Entity:
	...
	if room_node != null:
		for player_id in living_player_ids:
			...
			if player != null:
				if player_id != dont_target_player_w_id and Lobby.is_on_same_team_as(player_id, my_team_name) == false:
					...
					if is_bot == true:
						...
						if dist < closest_dist:
							...
							var see_transformed_player:bool = Util.rand.randf() > 0.9 and dist < 15 or player.get_move_dir_v3() != Vector3.ZERO
							if see_transformed_player or spell_caster.is_transformed == false:
								closest_dist = dist
...
func get_living_player_with_index(index:int)->Entity:
	if index < living_player_ids.size() and index >= 0:
...
func p2p_handle_player_death(id:int, respawn_time:float, death_type:int)->void :
	...
	if remove_living_at != - 1:
		...
		if player_component != null and spell_caster != null:
...
func on_player_cast_spell(caster_entity:Entity, spell_type:int)->void :
	if player_ids.find(caster_entity.get_id()) != - 1:
		if spell_type != Globals.SpellTypes.DIVE and spell_type != Globals.SpellTypes.DASH and spell_type != Globals.SpellTypes.DASH_BEAM and spell_type != Globals.SpellTypes.HEAL:
...
func spawn_player(id:int, player_info:Dictionary)->void :
	...
	var should_update_transform_locally:bool = (Lobby.SECURE_GAME == true and Lobby.is_host or player_type != Globals.PlayerTypes.CLIENT and Lobby.is_host or Lobby.SECURE_GAME == false and is_clients_player == true)
