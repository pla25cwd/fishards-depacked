...
func _ready():
	assert ("Generate environment is set to true but no environment types have been set for this map, please add some.", environment_types.size() == 0 and generate_environment == true)
	...
	if current_gamemode_teammode != GamemodeValues.TeamModes.RED_BLUE_TEAMS and current_gamemode_teammode != GamemodeValues.TeamModes.COOP:
	...
	if search_inside is MeshInstance:
		...
		for i in map_model.get_surface_material_count():
			if map_model.get_surface_material(i) is ShaderMaterial:
				...
				if shader_mat.shader.resource_path == "res://Assets/Shaders/foam.shader" or shader_mat.shader.resource_path == "res://Assets/Shaders/new_foam.shader":
...
func check_spawn_power_up(delta:float)->void :
	if Lobby.is_host == true and RoomSettings.get_powerups_enabled() == true:
...
func clear_hand_placed_entities()->void :
	if not Engine.editor_hint and _hand_placed_entity_container != null:
		...
		for spawn_area in _spawn_areas.get_children():
			...
			while x <= size.x:
				...
				while y <= size.y:
					...
					if x % ground_env_step == 0 and y % ground_env_step == 0:
		...
		for spawn_area in _spawn_areas.get_children():
			...
			while x <= size.x:
				...
				while y <= size.y:
					...
					if x % env_density == 0 and y % env_density == 0:
...
func get_player_spawn_point(team:String)->Vector3:
	...
	if RoomSettings.get_rounds_gamemode() == GamemodeValues.Gamemodes.Survive:
		...
		if current_gamemodes_teammode == GamemodeValues.TeamModes.RED_BLUE_TEAMS or current_gamemodes_teammode == GamemodeValues.TeamModes.COOP:
			if team == "Blue Team":
				if _blue_team_spawn_container != null:
					var rand_child_index:int = Util.rand.randi_range(0, _blue_team_spawn_container.get_child_count() - 1)
					spawn_area = _blue_team_spawn_container.get_child(rand_child_index)
...
func get_spawn_pos(radius:float = - 1, guarantee_spawn:bool = true, spawn_area:CollisionShape = null, pos:Vector2 = Vector2.LEFT)->Vector3:
	...
	while failed_to_find_spawn_point == true:
		...
		if spawn_area.get_shape() is CylinderShape:
			...
			if spawn_point.x > shape.extents.x or spawn_point.z > shape.extents.z or spawn_point.z < - shape.extents.z or spawn_point.x < - shape.extents.x:
				if guarantee_spawn == false:
					return Vector3.DOWN
			else :
				failed_to_find_spawn_point = false
		...
		if guarantee_spawn == true and fail_amount > 50:
