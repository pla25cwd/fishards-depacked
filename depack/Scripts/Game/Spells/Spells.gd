...
func get_closest_enemy_totem(my_pos:Vector3, my_team_name:String):
	...
	for spell_id in spell_ids:
		...
		if spell != null:
			if spell.get_subtype() == Globals.SpellTypes.TOTEM and Lobby.is_on_same_team_as(spell.get_creator_id(), my_team_name) == false:
	...
	if Globals.game_paused == false:
		...
		if spell_instance != null:
			...
			if damage_comp != null:
				...
				if caster_entity.get_type() == Globals.EntityTypes.SPELL and caster_entity.get_subtype() == Globals.SpellTypes.TOTEM:
