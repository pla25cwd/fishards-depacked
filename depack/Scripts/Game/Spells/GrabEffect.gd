...
func _on_grab_area_collided(area)->void :
	...
	if grabbed_entity != null:
		...
		if (get_node(grab_hit_path).is_playing() == false and grabbed_entity.get_id() != parent_entity.get_creator_id()):
			get_node(grab_hit_path).play()
		
		if grabbed_entity.get_type() == Globals.EntityTypes.ENVIRONMENT and grabbed_entity.get_subtype() == Globals.EnvironmentTypes.FISH_BALL:
			var res = parent_entity.transform.basis.z * - 5
			res.y = 0
			grabbed_entity.add_force(res)
		elif grabbed_entity.get_type() == Globals.EntityTypes.ENVIRONMENT and grabbed_entity.get_subtype() != Globals.EnvironmentTypes.DUMMY and grabbed_entity.get_subtype() != Globals.EnvironmentTypes.STRONG_DUMMY:
			hit_solid(area)
		elif grabbed_entity.get_type() == Globals.EntityTypes.PLAYER and grabbed_entity.get_id() != parent_entity.get_creator_id():
			var added = try_add_to_grabbed(grabbed_entity)
			if added == true and parent_entity.get_creator_id() == SteamValues.STEAM_ID:
...
func try_add_to_grabbed(entity:Entity)->bool:
	...
	if grabbed_entities_ids.size() == 0 and grabbed_entities_ids.find(id) == - 1 and entity.is_grabbed == false:
