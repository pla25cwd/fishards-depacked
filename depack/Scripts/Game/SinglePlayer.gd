...
func _on_lobby_chat(_sender:String, message:String)->void :
	if message.begins_with("/dummy") and RoomSettings.get_gamemode() == GamemodeValues.Gamemodes.Training:
		_dummy_mode = not _dummy_mode
		if _dummy_mode == true:
			clear_temp_bots()
...
func _on_player_killed(killed_id, killer_id, killed_with_spell):
	if killed_id == SteamValues.STEAM_ID:
		...
		if _tutorial_final_challenge == true and _dummy_mode == false:
...
func _on_entity_destroyed(entity_id:int):
	if entity_id == training_dummy_id and _dummy_mode == true:
		create_training_dummy()
