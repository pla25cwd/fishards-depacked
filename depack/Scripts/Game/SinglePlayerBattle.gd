...
func _on_enter_area_entered(area):
	if spawned_bots == false and defeated_battle == false:
		var enter_entity:Entity = Util.get_entity_from_area(area)
		if enter_entity != null:
			if enter_entity.get_type() == Globals.EntityTypes.PLAYER and enter_entity.get_subtype() == Globals.PlayerTypes.CLIENT:
				spawn_bots()
...
func _on_player_killed(killed_id, killer_id, killed_with_spell):
	...
	if bot_ids.empty() == true and spawned_bots == true and defeated_battle == false:
		fight_over()
...
func _on_respawn_player(player_id)->void :
	if player_id == SteamValues.STEAM_ID and defeated_battle == true:
