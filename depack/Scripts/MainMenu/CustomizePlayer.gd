...
func apply_skin_to_fishard(entity:Entity, apply_skin:Dictionary, fishard_model:Spatial, team_name:String = "")->Dictionary:
	...
	if entity != null:
		ignore_team_color = entity.get_subtype() != Globals.PlayerTypes.CLIENT and entity.get_subtype() != Globals.PlayerTypes.EASY_BOT and entity.get_subtype() != Globals.PlayerTypes.MEDIUM_BOT and entity.get_subtype() != Globals.PlayerTypes.HARD_BOT
	...
	if team_name == "Red Team" and ignore_team_color == false:
		fishard_model.set_surface_material(2, palette_array[25])
		new_skin["skin"] = 25
	elif team_name == "Blue Team" and ignore_team_color == false:
