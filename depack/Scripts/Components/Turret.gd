...
func _process(delta)->void :
	if Lobby.is_host and RoomSettings.get_game_started() == true and not Globals.game_paused:
...
func _find_closest_player(delta:float):
	...
	if players_node != null and _hunt_counter > _hunt_max_time:
		_hunt_counter = 0
...
func _check_turret_cast_spell(delta:float)->void :
	...
	if _update_cast_spell_counter > cast_spell_time and _closest_player_id != - 1:
