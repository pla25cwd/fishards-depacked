...
func _on_entity_collided(area_or_body):
	if _collided == false:
		...
		if entity != null:
			...
			if on_same_team == false and entity.get_id() != parent_entity.get_creator_id() and entity.get_component_of_type(Globals.ComponentTypes.ReflectProjectiles) == null and (entity.get_type() != Globals.EntityTypes.SPELL or entity.get_type() == Globals.EntityTypes.SPELL and entity.get_subtype() == Globals.SpellTypes.TOTEM):
				PacketSender.host_broadcast_destroy_entity(parent_entity.get_id())
				_collided = true
				return 
