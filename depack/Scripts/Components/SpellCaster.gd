...
func _input(event):
	if parent_entity.is_my_client:
		if Globals.ui_interaction_mode == Globals.UIInteractionModes.GAMEPLAY and Input.is_action_just_pressed("cast_spell"):
...
func _process(delta:float)->void :
	if parent_entity.is_my_client:
		...
		if allowed_to_cast_spell(active_spell) == true and Input.is_action_pressed("cast_spell"):
		...
		if hold_down_spell == true and Input.is_action_just_released("cast_spell"):
			deactivate_spell(active_spell)
			if UserSettings.get_auto_clear_elements() == true:
				reset_elements(true)
		
		if Input.is_action_just_released("cast_spell") and active_spell != Globals.SpellTypes.NONE and _prev_cast_spell_input_successful == true:
...
func set_immobilized(val:bool, is_helpless:bool = false)->void :
	...
	if _is_helpless == true and val == false:
		return 
	
	Lobby.emit_signal("player_immobilized", parent_entity.get_id())
	if val == false and holding_down_spell and UserSettings.get_auto_clear_elements() == true:
		deactivate_spell(active_spell)
	_immobilized = val
...
func try_request_cast_spell(cast_pos:Vector3 = Vector3.DOWN, team_name:String = "", spell:int = active_spell, ignore_cooldown:bool = false)->bool:
	...
	var is_bot:bool = parent_entity.get_type() == Globals.EntityTypes.PLAYER and parent_entity.get_subtype() != Globals.PlayerTypes.CLIENT
	...
	if (gameplay_mode or is_bot):
		if check_cooldown_of_spell_over(spell) and casting_spell_w_cast_time == false and holding_down_spell == false:
	...
	if allowed_to_cast and parent_entity.is_my_client:
	...
	if allowed_to_cast == true or Lobby.is_host == false:
...
func activate_rock_dash(dir:Vector2)->void :
	...
	if parent_entity.get_type() == Globals.EntityTypes.PLAYER and _immobilized == false:
...
func _on_not_cast_spell_timeout()->void :
	if parent_entity.is_my_client and _knows_how_to_cast_spell == false:
		elementTip.show_tip()
...
func _on_entity_collided(area:Area)->void :
	...
	if hit_entity != null:
		...
		if hit_entity.get_type() == Globals.EntityTypes.SPELL:
			if hit_entity.get_subtype() != Globals.SpellTypes.ARCANE_WALL and hit_entity.get_subtype() != Globals.SpellTypes.TOTEM:
				return 
...
func _on_lobby_members_updated()->void :
	for player_info in Lobby.get_all_lobby_player_info():
		...
		if player_info["id"] == parent_entity.get_id() and Lobby.player_is_in_temp_bot(player_info["id"]) == false:
...
func _on_took_damage(_damage:float, _damager_entity:Entity)->void :
	...
	if Lobby.is_host and is_transformed == true and not _health_component.get_invincible():
...
func transform_into(transform_into:int, move_speed_change:float = 1.0, duration:float = - 1)->void :
	if _default_entity_model == null or _transform_container == null:
	...
	if transform_into == - 1 and is_transformed == true:
	...
	elif is_transformed == false and transform_into != - 1:
...
func update_active_spell(requested_spell:int = Globals.SpellTypes.NONE, delay_stance:bool = false, set_spell_without_correct_elements:bool = false)->void :
	...
	if new_spell == Globals.SpellTypes.NONE and UserSettings.get_auto_clear_elements() == false:
	...
	if previous_spell != new_spell:
		var is_client = (parent_entity.get_type() == Globals.EntityTypes.PLAYER and parent_entity.get_type() == Globals.PlayerTypes.CLIENT)
		if parent_entity.is_my_client or Lobby.is_host and is_client == false:
			deactivate_spell(previous_spell)


func update_player_speed_from_active_spell(is_transformed:bool)->void :
	
	if _player_component != null and is_transformed == false:
...
func get_spell_from_elements(firstEl:int, secondEl:int)->bool:
	if ((selected_elements[0] == firstEl and selected_elements[1] == secondEl)
	 or (selected_elements[0] == secondEl and selected_elements[1] == firstEl)):
		return true;
	else :
		return false;
...
func _update_cooldowns(delta:float)->void :
	for spell_id in _spell_cooldowns:
		...
		if _get_max_charges(spell_id) > 1 and _spell_cooldowns[spell_id].charge_regen_cooldown < 0:
			...
			if active_spell == spell_id and parent_entity.is_my_client and active_spell != Globals.SpellTypes.NONE:
				spellHUD.set_cooldown_values(_get_cooldown_progress_of_spell(active_spell), _get_max_cooldown_of_spell(active_spell), _get_charge_amount(active_spell), _get_max_charges(active_spell))
...
func check_show_hold_down_tip()->void :
	if _knows_how_to_hold_down == false and parent_entity.is_my_client and Globals.ui_interaction_mode == Globals.UIInteractionModes.GAMEPLAY:
...
func check_show_cast_tip(spell:int)->void :
	if parent_entity.is_my_client and Globals.ui_interaction_mode == Globals.UIInteractionModes.GAMEPLAY:
		amount_of_clicks += 1
		if spell == Globals.SpellTypes.NONE and _knows_how_to_cast_spell == false:
...
func deactivate_spell(spell_type:int = active_spell)->void :
	...
	if holding_down_spell == true and Globals.SpellCooldowns[spell_type]["hold_down"] == true:
		...
		if spell_type == Globals.SpellTypes.ICE_BEAM and _player_component != null:
			_player_component.remove_speed_modifier("icebeamslowdebuff")
		
		if spell_type == Globals.SpellTypes.WILDFIRE and _player_component != null:
...
func allowed_to_cast_spell(spell:int)->bool:
	return check_cooldown_of_spell_over(spell) == true and _immobilized == false and parent_entity.get_is_trapped() == false and Util.get_current_game_state() != GamemodeValues.GameStates.STARTING and Globals.ui_interaction_mode == Globals.UIInteractionModes.GAMEPLAY
...
func check_cooldown_of_spell_over(spell_type:int)->bool:
	return _spell_cooldowns[spell_type].cooldown_progress <= 0 and _get_charge_amount(spell_type) > 0
...
func get_child_spell(spell_type:int)->Entity:
	...
	if room_node != null:
		for id in _children_spell_ids:
			...
			if spell_entity != null:
				if spell_entity.get_type() == Globals.EntityTypes.SPELL and spell_entity.get_subtype() == spell_type:
					return spell_entity
...
func _on_child_spell_destroyed(entity_id:int)->void :
	...
	if child_index != - 1:
		...
		if room_node != null:
			...
			if spell_entity != null:
				if spell_entity.get_creator_id() == parent_entity.get_id() and Globals.SpellCooldowns[spell_entity.get_subtype()]["hold_down"] == true:
...
func _on_TransformDurationTimer_timeout():
	if Lobby.is_host and is_transformed == true:
		PacketSender.transform_into(parent_entity.get_id(), - 1)
