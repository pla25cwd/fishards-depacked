...
func _on_entity_collided(area:Area):
	...
	if hit_entity != null and Lobby.is_host:
		if power_type == power_types.HEALTH:
			...
			var player = hit_entity.get_component_of_type(Globals.ComponentTypes.Player)
			if my_team.empty() == false and player != null and players_node != null and Lobby.player_is_in_temp_bot(hit_entity.get_id()) == false:
