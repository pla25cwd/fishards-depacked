...
func _on_entity_collided(area:Area)->void :
	if area.get_parent() is Entity:
		...
		if hit_entity.get_id() != _dont_trap_player_with_id and not (hit_entity.get_type() == Globals.EntityTypes.PLAYER and hit_entity.get_subtype() == Globals.PlayerTypes.ICEARD):
