...
func _physics_process(delta)->void :
	...
	if Lobby.is_host:
		...
		if Util.safe_to_use(_closest_player) == true and _has_activated:
...
func _on_crab_dealt_damage(_amount:int, hit_entity:Entity)->void :
	...
	if hit_entity.get_type() == Globals.EntityTypes.SPELL and hit_entity.get_subtype() == Globals.SpellTypes.CRAB or hit_entity.get_type() == Globals.EntityTypes.ENVIRONMENT:
		return 
