...
func heal(entity:Entity)->void :
	var is_enemy = entity.get_type() == Globals.EntityTypes.PLAYER and not Lobby.is_on_same_team_as(entity.get_id(), heal_creator_team)
	if heal_enemies == false and is_enemy and parent_entity.get_creator_id() != entity.get_id():
		return 
	
	if heal_self == false and entity.get_id() == parent_entity.get_creator_id():
		return 
...
func _on_entity_entered_area(entering_area:Area):
	...
	if entity != null:
		...
		if heal_once_only and not entities_in_area.has(entity_id):
			heal(entity)
