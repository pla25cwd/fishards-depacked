...
func knockback_entity(hit_entity:Entity)->void :
	if hit_entity != null and _can_knockback == true and (hit_entity.get_id() != _dont_push_player_with_id or knockback_creator == true):
		
		if hit_entity.get_type() == Globals.EntityTypes.SPELL and knockback_spells == false:
			return 
		...
		if not knockback_teammates and hit_entity.get_type() == Globals.EntityTypes.PLAYER and Lobby.is_on_my_team(hit_entity.get_id(), parent_entity.get_creator_id()):
			return 
		...
		if parent_entity.get_creator_id() == SteamValues.STEAM_ID and parent_entity.get_type() == Globals.EntityTypes.SPELL and parent_entity.get_subtype() == Globals.SpellTypes.PUSH:
			AchievementHandler.inc_pushes()
		
		var force:Vector3
		if launch and Lobby.is_host:
...
func _on_entity_entered_area(area:Area):
	...
	if entity != null:
		...
		if entities_in_area.find(entity_id) == - 1 and _can_knockback == true:
