...
export (float) var repeat_time = - 1
...
func _ready():
	...
	if not affect_creator:
		_dont_affect_player_with_id = parent_entity.get_creator_id()


func _set_speed_modifier(entity:Entity)->void :
	if affect_teammates == false and affect_creator == false and entity.get_type() == Globals.EntityTypes.PLAYER and Lobby.is_on_my_team(entity.get_id(), parent_entity.get_creator_id()):
		return 
	...
	if entity.get_id() != _dont_affect_player_with_id and entity.get_type() == Globals.EntityTypes.PLAYER:
		if modifier_name != "":
...
func _on_entity_collided(area:Area)->void :
	...
	if hit_entity != null:
		...
		if affect_creator == false and hit_entity.get_id() == parent_entity.get_creator_id():
			return 
		...
		if freeze_on_collide == true:
			...
			if affect_teammates == false and Lobby.is_on_my_team(hit_entity.get_id(), parent_entity.get_creator_id()):
				return 
			
			for entity_type_data in _not_freezable_entites:
				if entity_type_data.type == hit_entity.get_type() and entity_type_data.subtype == hit_entity.get_subtype():
					return 
	...
	if repeat_time != - 1:
		...
		if room_node != null:
			for id in entity_ids_in_area:
				...
				if entity != null:
					var player = entity.get_component_of_type(Globals.ComponentTypes.Player)
					if modifier_name != "" and player != null:
						player.set_speed_modifier(modifier_name, speed_modifier, duration)
