...
func _ready():
	...
	if join_res != OK or join_req_res != OK or lbby_crt_res != OK or p2p_req_res != OK or chat_upt_res != OK or p2p_fail_res != OK or get_lobbies_res != OK:
		printerr("Error occured in one of gd's signals")


func get_must_wait_to_join()->bool:
	return SceneLoader.get_is_loading() == true or Util.get_current_game_state() == GamemodeValues.GameStates.ROUND_OVER
...
func _on_get_lobby_match_list(lobbies):
	if _searching_for_quickplay == true:
		...
		for lobby_id in lobbies:
			...
			if password_required == false and is_fishards == true:
...
func check_lobby_ping(delta:float)->void :
	...
	if time_since_last_ping > 2 and lobby_id != 0:
...
func handle_afk(delta:float)->void :
	if lobby_id != 0 and Globals.in_single_player == false:
		afk_time += delta
		if afk_time > 5 * 60:
			leave_lobby("afk")
...
func _on_lobby_joined(lobbyID, _permissions, _locked, response):
	...
	if response == Steam.CHAT_ROOM_ENTER_RESPONSE_SUCCESS:
		...
		if (Lobby.is_host or password_required == false):
...
func host_add_player_and_broadcast(player_info:Dictionary, is_bot:bool, is_temp_bot:bool)->bool:
	...
	if is_bot == false and is_temp_bot == false:
		client_members.append(player_info)
	elif is_bot == true and is_temp_bot == false:
...
func host_start_game()->void :
	if client_members.size() > 0 and is_host:
	...
	for i in client_members.size():
		if client_members[i].id == sender_id:
			...
			if kill_on_update == true:
				...
				if SceneLoader.player_has_loaded(sender_id) and room_node != null and Util.get_current_game_state() == GamemodeValues.GameStates.GAME:
					...
					if updater_entity != null:
						...
						if health_component != null and player_component != null:
...
func _on_P2P_session_connect_fail(lobbyID:int, session_error:int)->void :
	...
	if session_error == 0:
		...
		if lobby_id != 0 and is_host == false and lobbyID == host_id:
...
func is_on_my_team(my_id:int, their_id:int)->bool:
	for team_info in teams:
		if team_info["member_scores"].has(my_id) and team_info["member_scores"].has(their_id):
			return true
...
func is_on_same_team_as(their_id:int, my_teams_name:String)->bool:
	for team_info in teams:
		if team_info["member_scores"].has(their_id) and team_info.name == my_teams_name:
			return true
...
func add_player_to_team(current_round_team_mode:int, player_info:Dictionary)->void :
	...
	if player_info["clone"] != - 1:
		...
		if current_round_team_mode == GamemodeValues.TeamModes.NO_TEAMS:
			...
			solo_team = false
			if _temp_bot_members.find(player_info) == - 1:
	...
	var added_to_team:bool
	for i in teams.size():
		if teams[i]["name"] == to_team_w_name and teams[i]["member_scores"].has(player_info["id"]) == false:
