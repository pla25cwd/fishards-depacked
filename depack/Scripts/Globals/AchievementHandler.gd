...
func _ready():
	...
	_on_steam_stats_received(0, 0, 0)



func _input(event):
	if Input.is_key_pressed(KEY_C) and Globals.get_app_mode() == Globals.AppModes.DEVELOPMENT:
		for ach_name in steam_achievements:
	...
	if dir != Vector2.ZERO and dir != prev_joy_dir:
		prev_joy_dir = dir
...
func _on_round_won(is_winner):
	...
	var actually_played_the_game = true
	if round_aim_movement / (round_length_time + 0.01) < 2:
		actually_played_the_game = false
	
	if actually_played_the_game and is_winner and is_online:
...
func verify_got_achievements()->void :
	...
	if _falsely_achieved_achievement == true and _showed_popup_falsely_achieved == false:
...
func check_spell_achievement(value:int, ach_name:String, verify_aquired:bool = false)->void :
	if value >= 100:
		if steam_achievements[ach_name] == false and verify_aquired == false:
			store_achievement(ach_name)
...
func check_element_mode_win(element_mode:int, verify_aquired:bool = false)->void :
	if verify_aquired == false:
		if element_mode == Globals.ElementModes.RANDOM and achievements_data_dict["element_mode_rounds_won"].has(Globals.ElementModes.RANDOM):
			...
			if achievements_data_dict["element_mode_rounds_won"][Globals.ElementModes.RANDOM] >= 50:
				if steam_achievements[achievements.ACH_COMPLETE_WIN_50_ROUNDS_RANDOM_SAME] == false and verify_aquired == false:
		...
		if element_mode == Globals.ElementModes.TIMED and achievements_data_dict["element_mode_rounds_won"].has(Globals.ElementModes.TIMED):
			...
			if achievements_data_dict["element_mode_rounds_won"][Globals.ElementModes.TIMED] >= 50:
				if steam_achievements[achievements.ACH_COMPLETE_WIN_50_ROUNDS_RANDOM_TIME] == false and verify_aquired == false:
...
func check_class_win_achievement(class_id:int, updated_ach_data:Dictionary, verify_aquired:bool = false)->void :
	...
	if wins >= 30:
		if steam_achievements[achievement_name] == false and verify_aquired == false:
...
func store_achievement(name:String, set_steam_achievement:bool = true)->void :
	...
	if setSuccess and storeSucess and achievementHats.has(name) == true and set_steam_achievement == true:
