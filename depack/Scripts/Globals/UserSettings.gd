...
func check_first_time_playing()->void :
	if settings_dict["firstTimePlaying"] == true or Globals.get_app_mode() == Globals.AppModes.DEVELOPMENT:
...
func save_pb_training_time(time:int)->Dictionary:
	...
	if pb_training_time == - 1 or time > pb_training_time:
...
func save_and_broadcast_elements(elements:Array, kill_on_update:bool = true)->void :
	...
	if (Lobby.lobby_id != 0 or Globals.in_single_player == true):
...
func load_settings()->void :
	...
	for key in settings_dict["keybinds"].keys():
		set_key_bind(key, settings_dict["keybinds"][key], settings_dict["keybinds"][key] < 10)
	...
	if not actionlist.empty():
		if is_movement == false:
			for action in actionlist:
				if is_mouse == true and action is InputEventMouseButton:
					InputMap.action_erase_event(name, action)
					break
				elif is_mouse == false and action is InputEventKey:
					InputMap.action_erase_event(name, action)
					break
		else :
			for action in actionlist:
				if action is InputEventKey:
					if is_movement and (action.scancode == KEY_DOWN or action.scancode == KEY_UP or action.scancode == KEY_LEFT or action.scancode == KEY_RIGHT):
						continue
					InputMap.action_erase_event(name, action)
...
func discover_spell(spell:int)->void :
	if not settings_dict["spells_discovered"].has(spell):
